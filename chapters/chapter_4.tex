...
All simulation experiments are conducted in the Webots environment described in Chapter~\ref{chp:background}, using a single overhead light source with a known position as the navigation target.
...
Five methods---least squares plane fit, weighted least squares, robust RANSAC, local polynomial fit, and finite differences---are evaluated against the dense probe map, with weighted least squares yielding the lowest directional error.
