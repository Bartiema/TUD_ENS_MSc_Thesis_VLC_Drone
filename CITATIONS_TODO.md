...
These are citation fixes/additions for chapters **other than chapter_2** (Algorithm
Design), which we are not actively working on. Apply them when those chapters are in
scope. Chapter 2's own citation work is already done (see "Already applied" at the end).

All specific papers below are real, well-known works, but **verify exact volume / number /
pages before final submission.** Swap any reference for one you prefer.
...
### Low priority (arguably textbook common knowledge — optional)
| Location | Claim | Suggested |
|---|---|---|
| chapter_2.tex:90 | square-wave odd harmonics | `oppenheim1999dsp` (optional) |
| chapter_2.tex:194 | weighted circular mean via atan2 | `mardia2000directional` (optional) |
...
@book{oppenheim1999dsp,
	...
	publisher = "Prentice Hall", address = "Upper Saddle River, NJ, USA"
}
...
@book{mardia2000directional,
	...
	year = "2000", publisher = "Wiley", address = "Chichester, UK"
}
```
...
- **LOESS fix:** `\cite{cleveland1979loess}` → `\cite{bjorck1996lsq}` in chapter_2.tex,
  because the "Polynomial Surface" method is an unweighted ordinary-least-squares
  quadratic fit, not locally-weighted LOESS. Uses an existing key; no new entry.
- **chapter_2.tex:90** square-wave odd harmonics → `\cite{oppenheim1999dsp}` (entry
  ADDED to bib).
- **chapter_2.tex:194** weighted circular mean → `\cite{mardia2000directional}` (entry
  ADDED to bib).

> Note: `oppenheim1999dsp` and `mardia2000directional` are now in the bib, so the
> deferred chapter_4.tex:8 (Nyquist) cite can reuse `oppenheim1999dsp` without a new entry.
